...
"sdk")
	...
	;;
"toolchain")
	TOOLCHAIN_NAME="$(curl -A "$CURL_UA" -fsSL "$DOWNLOAD_URL/$DOWNLOAD_PATH/sha256sums" | grep -E "toolchain-(.*)${TARGET%-*}" | cut -d "*" -f 2)"
	curl --retry 3 --retry-all-errors --retry-delay 10 -A "$CURL_UA" -fLO "$DOWNLOAD_URL/$DOWNLOAD_PATH/$TOOLCHAIN_NAME"
	verify_shasum "$TOOLCHAIN_NAME"
	mkdir -p "sdk"
	tar -vxf "$TOOLCHAIN_NAME" -C "toolchain"/ --strip-components 1
	;;
*)
